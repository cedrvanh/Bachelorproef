<ul class="sidebar__menu nav my-3">
    <li class="nav-item__section">
        <h6 class="text-uppercase">Menu</h6>
    </li>
    <li class="nav-item">
        <a class="nav__link collapsed" data-toggle="collapse" data-target="#dropdown1">Routes</a>
        <div class="collapse" id="dropdown1" aria-expanded="false">
            <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link">
                        <span class="nav__link-text">Locations</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link">
                        <span class="nav__link-text">Cities</span>
                    </a>
                </li>
            </ul>
        </div>
    </li>
    <li class="nav-item">
        <a class="nav__link collapsed" data-toggle="collapse" data-target="#dropdown2">Characters</a>
        <div class="collapse" id="dropdown2" aria-expanded="false">
            <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link">
                        <span class="nav__link-text">Classes</span>
                    </a>
                </li>
            </ul>
        </div>
    </li>
    <li class="nav-item">
        <a class="nav__link" href="users">
            <span class="nav__link-text">Users</span>
        </a>
    </li>
    <li class="nav-item">
        <a class="nav__link">
            <span class="nav__link-text">Other</span>
        </a>
    </li>
</ul>